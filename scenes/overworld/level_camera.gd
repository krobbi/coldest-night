...
var _anchor_stack: Array[Node2D] = []
...
func _ready() -> void:
	set_process(false)
	EventBus.subscribe_node(EventBus.camera_set_limits_request, set_limits)
	EventBus.subscribe_node(EventBus.camera_follow_anchor_request, follow_anchor)
	EventBus.subscribe_node(EventBus.camera_unfollow_anchor_request, unfollow_anchor)
...
func unfollow_anchor() -> void:
	if _anchor_stack.is_empty():
		...
		position = _anchor.global_position
		_anchor = null
		_anchor_stack.remove_at(0)
		return
	
	_anchor_stack.remove_at(_anchor_stack.size() - 1)

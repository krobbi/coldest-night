...
func _init(tree_ref: SceneTree, config_ref: ConfigBus) -> void:
	_tree = tree_ref
	_config = config_ref
...
func _set_handling_resize(value: bool) -> void:
	...
	if value:
		if _tree.connect("screen_resized", self, "_apply_pixel_perfect") == OK:
			_is_handling_resize = true
		elif _tree.is_connected("screen_resized", self, "_apply_pixel_perfect"):

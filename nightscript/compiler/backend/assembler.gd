...
const STRATEGIES: Dictionary = {
	...
	IROp.JUMP_NOT_ZERO_LABEL: [LBL, OP, NightScript.JUMP_NOT_ZERO],
	IROp.CALL_FUNCTION_LABEL: [LBL, OP, NightScript.CALL_FUNCTION],
	...
	IROp.PUSH_INT: [INT],
	IROp.PUSH_STRING: [STR],
	IROp.LOAD_LOCAL_OFFSET: [INT, OP, NightScript.LOAD_LOCAL],
	IROp.STORE_LOCAL_OFFSET: [INT, OP, NightScript.STORE_LOCAL],
	...
	IROp.FORMAT_STRING: [OP, NightScript.FORMAT_STRING],
	...
	IROp.BEGIN_DIALOG_MENU: [OP, NightScript.BEGIN_DIALOG_MENU],
	IROp.STORE_DIALOG_MENU_OPTION_LABEL: [LBL, OP, NightScript.STORE_DIALOG_MENU_OPTION],
	IROp.END_DIALOG_MENU: [OP, NightScript.END_DIALOG_MENU],
			...
			while index < STRATEGIES[op.type].size():
				...
				match strategy:
					...
					INT:
						...
						buffer.put_32(op.int_value)
					STR:
						buffer.put_u8(NightScript.PUSH_STRING)
						buffer.put_32(get_string_id(op.str_value))
					LBL:
						buffer.put_u8(NightScript.PUSH_INT)
						buffer.put_32(pointers.get(op.str_value, 0))

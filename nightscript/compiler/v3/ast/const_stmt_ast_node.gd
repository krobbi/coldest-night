...
# Constant Declaration Statement AST Node
# A constant statement AST node is a structure used by the NightScript compiler
 #that represents a constant declaraton statement node of an abstract syntax
# tree.
...
# Set the constant declaration statement's expressions.
func _init(
		identifier_expr_ref: IdentifierExprASTNode, value_expr_ref: ExprASTNode).("Const") -> void:
	identifier_expr = identifier_expr_ref
	value_expr = value_expr_ref


# Get the constant declaration statement's children as an array.

...
# Get a 32-bit signed integer from the serial read stream.
func get_s32() -> int:
...
# Get a UTF-8 string with a 32-bit unsigned integer byte length from the serial
# read stream.
func get_utf8_u32() -> String:
	return get_data_u32().get_string_from_utf8()

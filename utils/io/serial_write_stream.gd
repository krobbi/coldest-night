...
# Put a 32-bit signed integer to the serial write stream.
func put_s32(value: int) -> void:
...
# Put a UTF-8 string with a 32-bit unsigned integer byte length to the serial
# write stream.
func put_utf8_u32(value: String) -> void:
	put_data_u32(value.to_utf8())
